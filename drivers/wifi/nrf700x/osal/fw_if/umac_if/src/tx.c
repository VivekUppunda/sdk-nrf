					      ...
					      void *nbuf)
{
	...
	config->raw_tx_info.frame_ddr_pointer =
				       ...
				       int peer_id)
{
	...
	nrf_wifi_osal_nbuf_data_pull(fmac_dev_ctx->fpriv->opriv,
                                     nwb,
                                     sizeof(struct nrf_wifi_fmac_rawpkt_info));
				     ...
				     unsigned char tx_desc_num)
{
	...
	if (pkts_pending) {
		...
		nrf_wifi_osal_log_err(fmac_dev_ctx->fpriv->opriv,
			...
			nrf_wifi_osal_log_err(fmac_dev_ctx->fpriv->opriv,
				      "%s: RAW TX packet pending - why??\n",
				      __func__);

			nrf_wifi_osal_mem_cpy(fmac_dev_ctx->fpriv->opriv,
			      &def_dev_ctx->raw_tx_config,
			      nwb,
			      sizeof(struct raw_tx_data));
						     ...
						     void *nwb)
{
	...
	nrf_wifi_osal_log_err(fmac_dev_ctx->fpriv->opriv,
			     "%s: packet parameters are data_rate = %d, packet_length from APP %d, tx_mode from APP = %d, queue from APP = %d\n",
			     __func__, def_dev_ctx->raw_tx_config.data_rate,
			     def_dev_ctx->raw_tx_config.packet_length, def_dev_ctx->raw_tx_config.tx_mode,
			     def_dev_ctx->raw_tx_config.queue);
