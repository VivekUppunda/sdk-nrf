...
void nrf_wifi_if_sniffer_rx_frm(void *os_vif_ctx, void *frm, struct raw_rx_pkt_header *raw_rx_hdr, bool promisc_mode) {
	...
	pkt = net_raw_pkt_from_nbuf(iface, frm, sizeof(struct raw_rx_pkt_header), raw_rx_hdr, promisc_mode);
	if (!pkt) {
		LOG_DBG("Failed to allocate net_pkt");
		return;
	}
	
//	LOG_ERR("Raw packet being sent to upper interface");
